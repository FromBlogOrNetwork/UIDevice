...
//  appKeyChain
//
//  Created by NegHao.W on 16/3/10.
//  Copyright © 2016年 NegHao.W. All rights reserved.
//

#import "NegHUUIDTool.h"
...
static const char kKeyChainUDIDAccessGroup[] = "com.312games.LiveStar.com.cnblogs.smileEvday";
@implementation NegHUUIDTool

+(NSString *)UUID{

    NSString *udid = [NegHUUIDTool getUDIDFromKeyChain];
    if (!udid) {
        NSString *sysVersion = [UIDevice currentDevice].systemVersion;
        if ([sysVersion integerValue]>= 7.0) {
            udid = [NegHUUIDTool _UDID_iOS7];
        }else if ([sysVersion integerValue]>= 2.0){
            udid = [NegHUUIDTool _UDID_iOS6];
        }
        [NegHUUIDTool settUDIDToKeyChain:udid];
    }
    return udid;
}
...
+ (NSString*)_UDID_iOS6
{
    return [NegHUUIDTool getMacAddress];
}
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([NegHUUIDTool getUDIDFromKeyChain]) {        // there is item in keychain
        [NegHUUIDTool updateUDIDInKeyChain:udid];
        [dictForAdd release];
        return YES;
    }
